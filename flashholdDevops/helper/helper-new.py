...
def deployDeleteTemplate(deploy_id):
    ...
    DeployHistory.objects.create(deploy=obj, msg=u"删除环境%s" % obj.envName,
...
def deployBaseApp(deploy_id):
    ...
    for image in images:
        ...
        try:
            ...
            dns.update(domain="{name}.{deploy_name}.k8s".format(
                name=image.appName, deploy_name=obj.envName))
        except Exception, e:
            print str(e)
            pass
    ...
    if charts:
        for chart in charts:
            ...
            if chart_name == "rmq":
                ...
                if chart_name == "mysql":
                    cmd = "helm install --repo {repo} {chart_name} --name {release_name} --namespace {namespace} --set persistence.existingClaim={namespace} --set images=harbor2.flashhold.com/library/flashhold-mysql --set imageTag=shentong-61723 || helm upgrade {release_name} {chart_name} --repo {repo} --set persistence.existingClaim={namespace} || helm delete --purge {release_name}\n".format(
                        repo=repo,
                        chart_name=chart_name,
                        release_name="%s-%s" % (obj.envName, chart_name),
                        namespace=obj.envName)
...
def deployDataset(deploy_id):
    obj = Deployment.objects.get(id=deploy_id)
    objv = CommomAppServicePortMapping.objects.filter(
        deploy=obj, commomAppName="mysql")[0]
    tmp_dir = "/tmp/%s" % (obj.envName)
    if not os.path.exists(tmp_dir):
        os.mkdir(tmp_dir)
    ...
    while (status != "Running"):
        ...
        time.sleep(6)
        if i == 9:
            rsn = os.system("echo 'mysql instance not running up in 30 seconds' >> {tmp_dir}/deploy_record.log 2>&1".format(
                deploy_file=sf_name, tmp_dir=tmp_dir))
            return tmp_dir, rsn
        i = i + 1
    ...
    cmd = "mysql -h {domain} -P {port} -uroot -p123456 < {f}".format(domain=(
        "%s.mysql.k8s.flashhold.com") % obj.envName, port=objv.randomPort, f=obj.dataSet.f.path)
    sf.write(cmd)
...
def deploy_tcp_mapping(deploy_id):
    ...
    obj.save()
    for k in rsn:
        featureServicePortMapping.objects.create(
            deploy=deploy, port=k, randomPort=rsn[k])
    return rsn
